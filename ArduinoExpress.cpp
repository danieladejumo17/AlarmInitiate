...
void RouteCallback::get(const String& path, void *(*middleware)(HTTP_Request&, HTTP_Response&), void *(*callback)(HTTP_Request&, HTTP_Response&))
{
    this->_path = path;
...
void RouteCallback::post(const String& path, void *(*middleware)(HTTP_Request&, HTTP_Response&), void *(*callback)(HTTP_Request&, HTTP_Response&))
{
    this->_path = path;
    this->_middleware = middleware;
    this->_callback = callback;
...
void RouteCallback::inspect(const String& path, void *(*middleware)(HTTP_Request&, HTTP_Response&), void *(*callback)(HTTP_Request&, HTTP_Response&))
{
    this->_path = path;
    this->_middleware = middleware;
    this->_callback = callback;
...
{
  if(this->_middleware) this->_middleware(req, res);
  if(this->_callback) this->_callback(req, res);  
}
...
void ArduinoExpressRouter::get(const String& path, void *(*callback)(HTTP_Request&, HTTP_Response&))
{
  get(path, nullptr, callback);
}


void ArduinoExpressRouter::get(const String& path, void *(*middleware)(HTTP_Request&, HTTP_Response&), void *(*callback)(HTTP_Request&, HTTP_Response&))
{
  addRouteCallback(RouteCallback{HTTP_Method::GET, path, middleware, callback});
}


void ArduinoExpressRouter::post(const String& path, void *(*callback)(HTTP_Request&, HTTP_Response&))
{
  post(path, nullptr, callback);
}


void ArduinoExpressRouter::post(const String& path, void *(*middleware)(HTTP_Request&, HTTP_Response&), void *(*callback)(HTTP_Request&, HTTP_Response&))
{
...
void ArduinoExpressRouter::use(const String &path, void *(*callback)(HTTP_Request&, HTTP_Response&, Next)) 
{
...
void ArduinoExpressRouter::use(void *(*callback)(HTTP_Request&, HTTP_Response&, Next))
{
  use("", callback);
}
